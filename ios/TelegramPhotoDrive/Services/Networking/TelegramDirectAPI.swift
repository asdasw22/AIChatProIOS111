...
final class TelegramDirectAPI {
    ...
    func upload(asset: BackupAsset, chatID: String, fileURL: URL, mimeType: String, wifiOnly: Bool) async throws -> TelegramDirectUploadResult {
        ...
        guard 200..<300 ~= http.statusCode else { throw TelegramDirectAPIError.badStatus(http.statusCode) }
        guard telegram.ok else { throw TelegramDirectAPIError.telegram(telegram.description ?? "Telegram رفض رفع الملف.") }
